...
public class HomeAsiaFragment extends BaseFragment {
    @BindView(R2.id.mRecyclerView)
    RecyclerView mRecyclerView;
    @BindView(R2.id.mSmartRefreshLayout)
    SmartRefreshLayout mSmartRefreshLayout;
    private List<Asia> datalist = new ArrayList<>();
    VideoListAdapter adapter;
    Unbinder unbinder;
    @Override
    public int initLayout() {
        return R.layout.fragment_home_asia;
    }
    ...
    public void initView(View view) {
        ...
        adapter = new VideoListAdapter(datalist,getContext());
    ...
    public void initData() {
        ...
        mSmartRefreshLayout.setOnRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh(RefreshLayout refreshLayout) {
                BmobQuery<Asia> bmobQuery = new BmobQuery<>();
                bmobQuery.setLimit(10);
                bmobQuery.setSkip(0);
                bmobQuery.order("-createdAt");
                bmobQuery.findObjects(new FindListener<Asia>() {
                    @Override
                    public void done(List<Asia> list, BmobException e) {
                        if (e==null){
                            datalist.clear();
